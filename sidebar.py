...
def render_session_item(session):
    ...
    with st.container():
        ...
        with delete_col:
            if st.button("🗑️", key=f"delete_{session_id}", help="Delete this session", use_container_width=True):
        ...
        if st.session_state.delete_confirmation == session_id:
            ...
            with left_btn:
                if st.button("Yes", key=f"confirm_delete_{session_id}", use_container_width=True):
